import { normalizeBriefSources, type BriefSourceInput } from "@proof/shared";
import { extractValidUrl } from "../lib/url";

export function FooterSources({ sources }: { sources?: BriefSourceInput[] }) {
  const normalizedSources = normalizeBriefSources(sources)
    .map((source) => ({
      ...source,
      url: extractValidUrl(source.url) ?? ""
    }))
        ...
        {normalizedSources.map((s, i) => (
          ...
          >
            ...
            <span className="flex-1 truncate text-foreground font-medium group-hover:text-primary transition-colors">
              {s.title ? s.title : getDomain(s.url)}
            </span>
